...
| `category` | `QuestCategory` | `@default(LONG_TERM)` | "Quest Board" grouping - see enum reference. Not nullable; every quest has a real category. |
...
"Quest Board" grouping (Feature 2) - every quest has exactly one.

| Value | Meaning |
| --- | --- |
| `DAILY` | Short-term, same-day scope. |
| `WEEKLY` | Larger, week-scoped objective. |
| `LONG_TERM` | Major, open-ended objective (the schema default - existing quests before this feature all backfilled here). |
| `SYSTEM` | Auto-generated by `QuestsService.ensureSystemQuest` (not user-created, though nothing stops a user from picking it manually too) - see `docs/gameplay-systems.md`. |
