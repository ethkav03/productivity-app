...
| `/recommendations` | `frontend/app/(app)/recommendations/page.tsx` | New route (Sprint 7, "Intelligence"): a grid of `GET /recommendations`'s cards (empty state if none apply), each with a per-type icon and, where the card carries an ID, a link to the relevant attribute/skill/goal page (or `/quests` for the two quest-related types, since there's no single-quest detail route). Below that, a "Weekly Review" card rendering `GET /recommendations/weekly-review` - XP this week vs. last week with a delta badge, quests/habits completed, day streak, most-improved skill, and quietest attribute. |
...
streak-protection-charges, and then calls `celebrate(result)`. The goal detail page's milestone
"mark complete" mutation (Sprint 4) also calls `celebrate()`, but only when the response includes
a `completion` - a zero-`xpReward` milestone's `PATCH` response has none, so completing a plain
checklist item stays silent (no XP toast for XP that was never awarded).
